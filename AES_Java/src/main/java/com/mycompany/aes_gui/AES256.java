...
public class AES256 {
    ...
    public static String[][] convertToMaxtrix4x8(byte[] input) {
        int index = 0;
        String[][] state = new String[4][8];
    ...
    public static String[][] keyExpansion(String[][] initialisationKey) {
        ...
        String[][] expandKey = new String[4][60];
        String[] temp = new String[4];
        String[] word = new String[4];
        String[] w = new String[4];

        // Sao chép khóa gốc vào 8 cột đầu của expandKey
        for (int i = 0; i < 4; i++) {
            for (int j = 0; j < 8; j++) {
...
//                }
                ...
                for (int j = 0; j < 4; j++) {
                    temp[j] = expandKey[j][col - 1]; // Từ cuối cùng của vòng trước
                    w[j] = expandKey[j][col - 8]; // Từ thứ 8 trước đó
                }
                temp = subWord(temp); // Thay thế với S_BOX
                temp = xorHex(temp, w); // XOR với từ 8 cột trước
            }
            else {
                // XOR từ hiện tại với từ 8 cột trước
                for (int j = 0; j < 4; j++) {
                    temp[j] = expandKey[j][col - 1];
                    w[j] = expandKey[j][col - 8];
                }
    ...
    public static String encrypt_hex(String plaintext, String inputKey) {
        ...
        System.out.println();
        String[][] originalKey = convertToMaxtrix4x8(inputKey.getBytes());
        ...
        for(int v = 0; v < inputEncrypt.length; v += 16) {
            ...
            displayMatrix2D(state);
            System.out.println("Used Key:");
            String[][] keyRound14 = new String[4][4];
            for(int row = 0; row < 4; row++) {
                for(int col = 56; col < 60; col++) {
                    keyRound14[row][col % 4] = key[row][col];
                }
            }
            displayMatrix2D(keyRound14);
            state = addRoundKey(state, keyRound14);
            System.out.println("State after AddRoundKey:");
            displayMatrix2D(state);
    ...
    public static String decrypt_hex(String ciphertext, String inputKey) {
        ...
        for(int v = 0; v < ciphertext.length(); v += 32) {
            ...
            for (int i = 0; i < 14 - 1; i++) {
                ...
                String[][] keyUsed = new String[4][4];
                for(int row = 0; row < 4; row++) {
                    for(int col = 56 - 4 * (i + 1); col < 56 - 4 * (i + 1) + 4; col++) {
                        keyUsed[row][col % 4] = key[row][col];
                    }
                }
                displayMatrix2D(keyUsed);
            ...
            for(int row = 0; row < 4; row++) {
                for(int col = 0; col < 4; col++) {
                    keyRound14[row][col] = key[row][col];
                }
            }
            displayMatrix2D(keyRound14);
            state = addRoundKey(state, keyRound14);
    ...
    public static void main(String[] args) {
        String plaintext = "Tôi là Phạm Việt Long";
        String key = "phamvietlong1504phamvietlong1504";
